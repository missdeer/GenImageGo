...
func TestDailyLoginPoints(t *testing.T) {
	db, err := model.InitDB("sqlite", ":memory:")
	if err != nil {
		t.Fatalf("failed to init db: %v", err)
	}
	...
	db.Model(&model.User{}).Where("id = ?", user.ID).Update("email_verified", true)

	loggedUser, _, err := svc.Login("points@example.com", "Password123")
	if err != nil {
...
func TestDailyLoginPointsUnverifiedUser(t *testing.T) {
	db, err := model.InitDB("sqlite", ":memory:")
	if err != nil {
		t.Fatalf("failed to init db: %v", err)
	}
	...
	loggedUser, _, err := svc.Login("unverified@example.com", "Password123")
	if err != nil {
		t.Fatalf("login failed: %v", err)
	}
	if loggedUser.Points != 0 {
		t.Errorf("expected 0 points for unverified user, got %d", loggedUser.Points)
	}
}
